...
//          postMessage({ method: scopeMethod('__ready'), params: "ping" }, true);
  ...
  function create_iframe() {
    try {
      if (!commChan) {
        ...
        iframe.src = "http://blogch.at/iframe";
        commChan = Channel.build({
          debugOutput: true,
          window: iframe.contentWindow,
          origin: "http://blogch.at",
          scope: "blogchat",
          onReady: function() {
            // do stuff?
          }
  ...
  window.BlogChat = {
    comments: function(pageName, callback) {
      if (!callback) { callback = pageName; pageName = null; }
